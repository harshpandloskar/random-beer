...
class BreweryList extends Component {
  ...
  getCountryCodeList() {
    axios({
      method: "GET",
      url:
        "http://localhost:3000/locations/?key=a5c1b917e7ba62dcd79f434ed73bc72d"
    })
      .then((res) => {
        let code = [
  ...
  getBreweriesList() {
    axios({
      method: "GET",
      url: `http://localhost:3000/locations/?countryIsoCode=${this.state.select.selectedCode}&order=breweryName&key==a5c1b917e7ba62dcd79f434ed73bc72d`
    })
      .then((res) => {
        this.setState({
    ...
    return (
      <div className="brewery-list-page">
        ...
        {this.state.breweries.map((item) => (
          <div key={item.id}>
            <Link to={`breweries/brewery/${item.breweryId}`}>
              {" "}
